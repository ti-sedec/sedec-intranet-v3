...
export function DetailHeader({
  ...
  return (
    <>
      ...
      <header className="max-w-[820px] mx-auto px-6 pt-6.5 pb-2">
        <span className="font-mono text-[11px] font-semibold tracking-[0.14em] uppercase" style={{ color: "var(--color-accent-strong)" }}>
          {kicker}
        </span>
        <h1
          className="font-[family-name:var(--font-display)] font-extrabold m-0 mt-3.5 mb-4 text-[var(--color-ink)]"
          style={{ fontSize: "clamp(30px,4.6vw,54px)", lineHeight: 1.04, letterSpacing: "-1.5px" }}
        >
          {title}
        </h1>
        {dek && (
          <p className="m-0 mb-5.5 text-[var(--color-ink-soft)]" style={{ fontSize: "clamp(17px,2vw,21px)", lineHeight: 1.5 }}>
            {dek}
          </p>
        )}
        {meta && (
